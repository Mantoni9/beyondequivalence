...
# Local (non-Zenodo) datasets: name -> (source, target, reference), relative to
# the project root. These bypass benchmark.zip entirely. The VDI→eBay gold case
# lives in the repo (goldstandard_ebay/); its two reference variants follow the
# delivery's eval protocol: 'vdi-ebay' evaluates against the 451 tier=seed
# concept pairs (Recall@K/MRR — the deductive closure would flood recall with
# trivial ancestor pairs), 'vdi-ebay-full' against all 2,055 closure rows
# (precision-style / hierarchy-credit analyses). Regenerate the .rdf files with
# goldstandard_ebay/make_reference.py after any gold update.
LOCAL_DATASETS: dict[str, tuple[str, str, str]] = {
    "vdi-ebay": (
        "goldstandard_ebay/vdi/vdi_karosserie_source_pos.owl",
        "goldstandard_ebay/ebay_kfz_target.owl",
        "goldstandard_ebay/reference_seed.rdf",
    ),
    "vdi-ebay-full": (
        "goldstandard_ebay/vdi/vdi_karosserie_source_pos.owl",
        "goldstandard_ebay/ebay_kfz_target.owl",
        "goldstandard_ebay/reference_full.rdf",
    ),
}
...
def load_subdataset(
    ...
    Names in LOCAL_DATASETS resolve to in-repo files (no benchmark.zip needed).
    """
    if name in LOCAL_DATASETS:
        root = _project_root()
        paths = tuple(root / rel for rel in LOCAL_DATASETS[name])
        missing = [str(p) for p in paths if not p.is_file()]
        if missing:
            raise FileNotFoundError(
                f"Local dataset '{name}' is missing file(s): {missing}. "
                "Reference files are generated by goldstandard_ebay/make_reference.py."
            )
