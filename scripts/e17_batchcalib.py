...
# llama g5/g7 reused the frozen Stage-1 baseline cells (dated dirs), which the
# matrix_<model>_<ds>_seed42_* glob below does NOT match. Verification scored
# these dirs' predictions directly, so the analysis must resolve to the SAME
# dirs or llama's g5/g7 silently drop out of its OAEI mean.
REUSED_CELLS = {
...
def resolve_cell(model, ds, rr):
    ...
    if ru and os.path.isfile(f"{rr}/{ru}/predictions.tsv"):
        return Path(rr)/ru/"predictions.tsv"
