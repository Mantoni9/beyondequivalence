...
class AlignmentHandler(object):
    ...
    def start(self, name, attrs):
        if name == self.base + "entity1":
            self.one_cell[0] = attrs[self.rdf + "resource"]
        elif name == self.base + "entity2":
            self.one_cell[1] = attrs[self.rdf + "resource"]
        elif name == self.base + "Ontology":
            self.onto_temp[0] = attrs[self.rdf + "about"]
...
def _remove_cell_extensions(alignment):
    for c in alignment:
        c.pop()
...
def parse_mapping_from_string(s, parse_cell_extensions=False):
    ...
    if parse_cell_extensions == False:
        _remove_cell_extensions(handler.alignment)
...
class Alignment:
    def __init__(self, filepath: str = None): 
        ...
        if filepath:
            alignment, _, _, _ = parse_mapping_from_file(filepath)
            for source, target, relation, confidence in alignment:
                self.add(Correspondence(source, target, relation, confidence))

    def add(self, r: Correspondence):
        k = r.key()
        if k in self._d:
            if r.confidence > self._d[k].confidence:
                self._d[k].confidence = r.confidence
        else:
            self._d[k] = r
            self._by_source[r.source].append(r)
            self._by_target[r.target].append(r)

    def find_by_source(self, source: URIRef) -> list[Correspondence]:
        return list[Correspondence](self._by_source[source])
    
    def find_by_target(self, target: URIRef) -> list[Correspondence]:
        return list[Correspondence](self._by_target[target])
    
    def sort_by_confidence(self):
        return sorted(self._d.values(), key=lambda r: (-r.confidence, r.source, r.target))

    def get_distinct_confidences(self):
        return set(r.confidence for r in self._d.values())

    def update(self, rs: Iterable[Correspondence]):
        for r in rs:
            self.add(r)

    def discard(self, r: Correspondence):
        if self._d.pop(r.key(), None) is not None:
            src_list = self._by_source.get(r.source)
            if src_list:
                src_list.remove(r)
            tgt_list = self._by_target.get(r.target)
            if tgt_list:
                tgt_list.remove(r)

    def difference_update(self, rs: Iterable[Correspondence]):
        for r in rs:
            self.discard(r)

    def remove_trivial(self):
        """Remove correspondences where source and target are the same."""
        to_remove = [r for r in self._d.values() if r.source == r.target]
        self.difference_update(to_remove)

    def __contains__(self, r: Correspondence): 
        return r.key() in self._d
    ...
    def __repr__(self):        
        return repr(set(self._d.values()))
    
    @classmethod
    def from_iterable(cls, rs: Iterable[Correspondence]):
        alignment = cls()
        alignment.update(rs)
        return alignment
